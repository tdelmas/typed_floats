...
Also, some methods are faster than the default implementation. For example, when possible `Eq` is impemented by coparing the bits of the two floats instead of the slower default implementation, that had special cases for `NaN` and `-0.0`.
